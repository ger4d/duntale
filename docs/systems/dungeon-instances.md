...
- Party state is in-memory, capped at 6 players including the owner, and wired into disconnect cleanup through `DuntalePlugin.onPlayerDisconnect(...)`. It seeds dungeon starts and can expand a current party into an active run during a floor transition; persisted instance membership remains the source of truth for re-entry and lifecycle lookups.
...
- Portal-triggered floor transitions are party-aware. The service first expands the trigger player's current party into the active instance using Continue-style rules: active members stay active, `LEFT` members are reactivated, new party members are added, and members in another safe active instance are migrated out of that prior instance. Prior instances in `CREATING` or `TRANSITIONING` block the transition before floor metadata changes.
- Runtime transfer is narrower than persisted membership. The plugin selects online active members currently in the old instance world, then adds online current-party members that are active after expansion even if they are standing in the village or another world. Offline members are not teleported.
- Before queuing the floor teleport, every selected runtime transfer player receives the detached transition camera through `ClickToMoveManager.prepareForWorldTransition(...)` on that player's current world thread. If the transition fails before transfer, prepared players still in the old world have dungeon overhead controls restored and the old floor portal is ensured.
- Operator `/dungeon transition <instanceId>` does not expand a party. It selects online active members currently in the old instance world, prepares that selected set with the transition camera, and fails clearly if nobody is eligible to transfer.
- Transitioning a floor claims `TRANSITIONING`, chooses the next floor's theme from the resolved `theme.variants` list, creates the next world, generates the next floor, teleports the selected runtime transfer set to the new entrance, arms the old world for removal when empty, and then re-persists the instance as `ACTIVE` on the new floor.
- Successful floor transitions advance the instance's floor metadata globally. Active members skipped from the runtime teleport, such as offline players, later route to the new floor through Continue or re-entry rather than preserving a per-player old-floor state.
