...
/**
 * Custom dungeon death page with paid continuation options and a free village retreat.
 */
public final class DungeonDeathPage extends InteractiveCustomUIPage<DungeonDeathPage.DeathEventData> {
    ...
    private static final String ACTION_LOWER_FLOOR = "lower-floor";
    ...
    public void build(
        ...
        boolean lowerDisabled = !context.lowerFloorAvailable() || context.balance() < context.lowerFloorCost();
        ...
        cmd.set("#LowerCostText.Text", context.lowerFloorAvailable()
                ? formatGold(context.lowerFloorCost()) + " gold"
                : "Unavailable");
        cmd.set("#LowerFloorText.Text", context.lowerFloorAvailable()
                ? "Restart on Floor " + (floorLevel - 1)
                : "Unavailable on Floor 1");
        cmd.set("#CurrentFloorButton.Disabled", currentDisabled);
        cmd.set("#LowerFloorButton.Disabled", lowerDisabled);
        ...
        events.addEventBinding(
                CustomUIEventBindingType.Activating,
                "#LowerFloorButton",
                EventData.of("Action", ACTION_LOWER_FLOOR)
        );
        events.addEventBinding(
                CustomUIEventBindingType.Activating,
                "#VillageButton",
    ...
    public void handleDataEvent(
        ...
        if (plugin == null) {
            LOGGER.at(Level.WARNING).log("Unable to handle dungeon death action because ZSquadPlugin is unavailable");
            return;
        }
        ...
        switch (data.action) {
            ...
            case ACTION_LOWER_FLOOR -> plugin.handleDungeonRespawnLowerFloor(ref, store, playerRef);
    ...
    private void disableActions() {
        ...
        cmd.set("#LowerFloorButton.Disabled", true);
