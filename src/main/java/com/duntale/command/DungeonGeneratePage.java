...
public class DungeonGeneratePage extends InteractiveCustomUIPage<DungeonGeneratePage.GenerateEventData> {
    ...
    private void runSummaryOnlyGeneration(@Nonnull DungeonConfig config) {
        ...
        GenerationOrchestrator orchestrator = DuntalePlugin.get().getDungeonOrchestrator();
    ...
    private CompletableFuture<PreparedPreviewWorld> preparePreviewWorld(@Nonnull DungeonConfig previewConfig) {
        return createPreviewWorld(previewConfig).thenCompose(previewWorld -> {
            ...
            return DuntalePlugin.get().getDungeonOrchestrator().generate(previewConfig)
    ...
    private CompletableFuture<World> createPreviewWorld(@Nonnull DungeonConfig previewConfig) {
        ...
        previewWorldConfig.setGameTime(WorldTimeResource.ZERO_YEAR.plus(16L, ChronoUnit.HOURS));
        previewWorldConfig.setGameplayConfig("Dungeon");
    ...
    private static Map<String, Object> loadSavedConfig() {
        Path configPath = DuntalePlugin.get().getDataDirectory().resolve(CONFIG_FILENAME);
    ...
    private static void saveConfig(@Nonnull GenerateEventData d) {
        try {
            Path dir = DuntalePlugin.get().getDataDirectory();
    ...
    public static class GenerateEventData {
        ...
        public static final BuilderCodec<GenerateEventData> CODEC = BuilderCodec.builder(
            ...
            // Action
            .append(new KeyedCodec<>("Generate", Codec.STRING), (e, v) -> e.generate = v, e -> e.generate)
            .add()
            // General
            .append(new KeyedCodec<>("@Seed", Codec.STRING), (e, v) -> e.seed = v, e -> e.seed)
            .add()
            .append(new KeyedCodec<>("@WorldName", Codec.STRING), (e, v) -> e.worldName = v, e -> e.worldName)
            .add()
            ...
            .add()
            .append(new KeyedCodec<>("@Assemble", Codec.BOOLEAN), (e, v) -> e.assemble = v, e -> e.assemble)
            .add()
            // Size
